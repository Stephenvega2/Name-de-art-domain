...
def generate_art():
    ...
    # Save the image with a default file name
    file_name = 'default_art'
    img.save(f'{file_name}.png')

    # Generate a unique HTTPS link using the predefined format
    unique_link = f'https://name*de**art+domain/{uuid.uuid4()}'
...
class ArtApp(App):
    def build(self):
        ...
        self.image = KivyImage(source='default_art.png')
    ...
    def update_image(self, instance):
        unique_link = generate_art()
        self.image.source = 'default_art.png'
        self.image.reload()
        print(f'Art saved as default_art.png with link: {unique_link}')
